// File: internal/adapter/persistence/wal_persister.go
package persistence
...
import (
	...
	// [SỬA] Import đúng engine và ports
...
// Struct nội bộ để lưu vào file (giữ nguyên cũng được)
...
// [SỬA] Dùng *engine.Store
func (w *WALPersister) Snapshot(s *engine.Store) error {
	...
	// Lưu ý: Đảm bảo trong internal/engine/store.go bạn đã có hàm SnapshotTo
...
// [SỬA] Dùng *engine.Store
func (w *WALPersister) Restore(s *engine.Store) error {
	...
	if _, err := os.Stat(snapshotPath); err == nil {
		...
		// Lưu ý: Đảm bảo trong internal/engine/store.go bạn đã có hàm RestoreFrom
	...
	for {
		...
		// Lưu ý: Đảm bảo internal/engine/store.go có hàm Put
